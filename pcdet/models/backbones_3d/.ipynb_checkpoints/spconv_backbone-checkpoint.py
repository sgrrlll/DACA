...
class VoxelBackBone8x(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, num_frames, **kwargs):
        ...
        self.conv5 = spconv.SparseSequential(
            # [200, 176, 6] <- [100, 88, 3]
            block(num_filters[3], num_filters[4], 3, norm_fn=norm_fn, stride=2,
                  padding=1, indice_key='spconv5', conv_type='spconv'),
            block(num_filters[4], num_filters[4], 3, norm_fn=norm_fn, padding=1, indice_key='subm5'),
            block(num_filters[4], num_filters[4], 3, norm_fn=norm_fn, padding=1, indice_key='subm5'),
        )

        self.conv6 = spconv.SparseSequential(
            # [200, 176, 6] <- [100, 88, 3]
            block(num_filters[4], num_filters[4], 3, norm_fn=norm_fn, stride=2,
                  padding=1, indice_key='spconv6', conv_type='spconv'),
            block(num_filters[4], num_filters[4], 3, norm_fn=norm_fn, padding=1, indice_key='subm6'),
            block(num_filters[4], num_filters[4], 3, norm_fn=norm_fn, padding=1, indice_key='subm6'),
        )
