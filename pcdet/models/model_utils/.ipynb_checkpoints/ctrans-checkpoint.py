...
class CoshMultiHeadAttention(nn.Module):
    def __init__(self, hidden_dim,num_heads):
        super(CoshMultiHeadAttention, self).__init__()
    ...
    def forward(self, query, key, value,batch_size, seq_len):
...
#         value = value.view(batch_size, seq_len, self.num_heads, self.hidden_dim // self.num_heads)
        ...
        # 转置以便进行注意力计算
#         query = query.transpose(1, 2)  # [batch_size, num_heads, seq_len, hidden_dim // num_heads]
#         key = key.transpose(1, 2)  # [batch_size, num_heads, seq_len, hidden_dim // num_heads]
#         value = value.transpose(1, 2)  # [batch_size, num_heads, seq_len, hidden_dim // num_heads]
#         print('query:', query.shape)
        ...
        #
        # # 加权求和
        # weighted_sum = torch.matmul(attention_weights, value)  # [batch_size, num_heads, seq_len, hidden_dim // num_heads]

        #双曲余弦
        query_cosh = query*cosh(1.1 / 384)
        query_sinh = query*sinh(1.1 / 384)
        key_cosh = query*cosh(1.1 / 384)
        key_sinh = query*sinh(1.1 / 384)
#         query_1 =query
#         key_1 =key
#         for i in range(seq_len):
#             query_1 = multiply_row(query_1,cosh(1.1*i / 384))
#             key_1 = multiply_row(key_1,cosh(1.1*i / 384))
              #query_1(:,i,:) = query_1(:,i,:)*cosh(1.1*i / 384)   看这个-----------------

#         query_cosh = query_1
#         key_cosh = key_1
# #         print('query_cosh:', query_cosh.shape)

#         for i in range(seq_len):
#             query = multiply_row(query,sinh(1.1*i / 384))
#             key = multiply_row(key,sinh(1.1*i / 384))
#         query_sinh = query
#         key_sinh = key
        QQQQ = torch.matmul(query_cosh,torch.matmul(key_cosh.transpose(-2, -1),value))
        VVVV = torch.matmul(query_sinh,torch.matmul(key_sinh.transpose(-2, -1),value))
#         print('qqqq:', QQQQ.shape)
#         print('vvvv:', VVVV.shape)


        weighted_sum = 2*torch.matmul(query,torch.matmul(key.transpose(-2, -1),value))-QQQQ-VVVV
#         print('weighted_sum:', weighted_sum.shape)

        # 将多个头的结果拼接起来
#         weighted_sum = weighted_sum.transpose(1, 2)  # [batch_size, seq_len, num_heads, hidden_dim // num_heads]
#         print('weighted_sum_1:', weighted_sum.shape)
#         weighted_sum = weighted_sum.contiguous().view(batch_size, seq_len, -1)  # [batch_size, seq_len, hidden_dim]
#         print('weighted_sum_2:', weighted_sum.shape)
...
#         self.att = CoshMultiHeadAttention(hidden_dim, 4)
...
#         V = torch.relu(V)
        ...
        out = self.att(Q, K, V)
#         out = self.att(Q, K, V, batch_size, seq_len)
...
class Attention_Layer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return out
#########我加的
def multiply_row(matrix, constant):
    row = matrix.shape[2]
    for i in range(row):
        row_l = matrix[:, :, i]  # 提取第i行
        multiplied_row = row_l * constant  # 与常数相乘
        matrix[:, :, i] = multiplied_row  # 将结果替换回原矩阵的第i行
    return matrix
