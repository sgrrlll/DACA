...
def separate_foreground(x, batch_dict, voxel_stride, voxel_size, point_cloud_range):
    ...
    new_x = spconv.SparseConvTensor(features[box_of_pts_cls_targets], indices[box_of_pts_cls_targets], x.spatial_shape,
                                    x.batch_size)
...
def split_voxels(x, b, offsets_3d, voxels_3d, kernel_offsets, mask_multi=True, topk=True, threshold=0.5):
    ...
    spatial_indices = (selected_indices_with_offset[:, 0] > 0) * (selected_indices_with_offset[:, 1] > 0) * (
                selected_indices_with_offset[:, 2] > 0) * \
                      (selected_indices_with_offset[:, 0] < x.spatial_shape[0]) * (
                                  selected_indices_with_offset[:, 1] < x.spatial_shape[1]) * (
                                  selected_indices_with_offset[:, 2] < x.spatial_shape[2])
    selected_indices_with_offset = selected_indices_with_offset[spatial_indices]
    selected_indices_with_offset = torch.cat(
        [torch.ones((selected_indices_with_offset.shape[0], 1), device=features_foreground.device) * b,
         selected_indices_with_offset], dim=1)

    selected_features = torch.zeros((selected_indices_with_offset.shape[0], features_ori.shape[1]),
                                    device=features_foreground.device)
    ...
    mask_voxel_kernel_selected = torch.cat(
        [torch.ones(features_foreground.shape[0], device=features_foreground.device), mask_voxel_kernel_selected],
        dim=0)

    # sort_by_indices()
    features_foreground_out, indices_foreground_coords_out, mask_voxel_kernel_selected_out = check_repeat(
        features_foreground_cat, indices_foreground_coords, additional_features=mask_voxel_kernel_selected)
