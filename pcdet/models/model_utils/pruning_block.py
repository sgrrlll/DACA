    ...
    def get_importance_mask(self, x, voxel_importance):
        ...
        for b in range(batch_size):
            ...
            if self.pruning_mode == "topk":
                batch_mask_position_idx = torch.argsort(batch_voxel_importance.view(-1, ))[
                                          :int(batch_voxel_importance.shape[0] * self.pruning_ratio)]
...
class SpatialPrunedConvDownsample(spconv.SparseModule):
    ...
    def gemerate_sparse_tensor(self, x, voxel_importance):
        ...
        voxel_indices_nim = []
        for b in range(batch_size):
            features_im, indices_im, features_nim, indices_nim = split_voxels_v2(x, b, voxel_importance,
                                                                                 self.kernel_offsets,
